...
module Jkf::Parser
  class Kif
        ...
        if s2 != :failed
          ...
          if s3 != :failed
            ...
            if s4 != :failed
              ...
              if s5 != :failed
                @reported_pos = s0
                s1 = { h: s1.join.to_i, m: s3.join.to_i, s: s5.join.to_i }
                s0 = s1
              else
                @current_pos = s0
                s0 = :failed
    ...
    def parse_ms
      ...
      if s1 != :failed
        ...
        if s2 != :failed
          ...
          if s3 != :failed
            @reported_pos = s0
            s1 = { m: s1.join.to_i, s: s3.join.to_i }
            s0 = s1
          else
            @current_pos = s0
            s0 = :failed
          ...
          if s3 != :failed
            ...
            if s4 != :failed
              ...
              if s5 != :failed
                ...
                if s6 != :failed
                  @reported_pos = s0
                  s1 = { te: s3.join.to_i, moves: s6[1..-1] }
                  s0 = s1
                else
                  @current_pos = s0
                  s0 = :failed
    ...
    def match_regexp(reg)
      ret = nil
      if @input[@current_pos] =~ reg
        ret = @input[@current_pos]
        @current_pos += 1
      else
        ret = :failed
